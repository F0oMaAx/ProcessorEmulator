...
public class Registers {
    ...
    public Registers() {
        ...
        String[] registers8H = {"AH", "BH", "CH", "DH"}; // Только для AX, BX, CX, DX

        // 2. Сегментные регистры
        String[] segmentRegisters = {"CS", "DS", "SS", "ES", "FS", "GS"};

        // 3. Указатель инструкций и флаги
        String[] instructionPointer = {"RIP"};
        String[] flags = {"RFLAGS"};

        // 4. Регистры управления (CR)
        String[] controlRegisters = {"CR0", "CR2", "CR3", "CR4", "CR8"};

        // 5. Регистры отладки (DR)
        ...
        // 7. Регистры SIMD (MMX, SSE, AVX, AVX-512)
        ...
        // 8. Специфические регистры модели (MSR)
        String[] modelSpecificRegisters = {"MSR"};

        // Заполняем все регистры значением 0
